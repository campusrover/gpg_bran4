...
void BrandeisServo::setup(String name, ros::NodeHandle &nh, Adafruit_PWMServoDriver ser_drr,
...
void BrandeisServo::setup_ease(double tar_angle, long curr_time_ms,
  ...
  LOG_INFO("setup_ease %f %d %f %f", target_angle, moving, change_in_value_angle, start_angle);
}
...
double BrandeisServo::compute_next_increment(long current_time_ms) {
  ...
  LOG_INFO("compute_next_increment %ld %.1f %.1f %.1f %.1f %.1f %1.f %.1f", current_time_ms, time_increment, ease_amount, start_angle, target_angle, new_angle, convert_scale, convert_offset);
